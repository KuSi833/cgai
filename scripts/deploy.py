...
class RemoteConfig:
    ...
    hostname: str
    remote_root: str
    uv_path: str = "/homes/$USER/.local/bin/uv"
...
git remote set-url origin https://$GITHUB_TOKEN@github.com/$GITHUB_USER/search-and-learn.git
...
export UV_PYTHON_INSTALL_DIR="/vol/bitbucket/$USER/.cache/python"
export HF_HOME="/vol/bitbucket/$USER/.cache/huggingface"
export UV_CACHE_DIR="/vol/bitbucket/$USER/.cache/uv"
...
def setup_remote(
    ...
    with console.status("[yellow]Cloning repository...", spinner="dots"):
        ...
        if result.failed:
            ...
            c.run(
                f"git clone git@github.com:{os.getenv('GITHUB_USER', 'YOUR_USERNAME')}/search-and-learn.git {remote_root}"
            )
    ...
    with console.status("[yellow]Syncing models directory...", spinner="dots"):
        c.run(
            f"rsync -avP /vol/bitbucket/$USER/search-and-learn/models/ {remote_root}/models/"
        )
...
def _checkout_commit(
    ...
    with console.status("[yellow]Fetching and checking out commit...", spinner="dots"):
        ...
        connection.run(
            f"git remote set-url origin https://{github_token}@github.com/$GITHUB_USER/search-and-learn.git",
            hide=True,
        )
...
def run_on_remote(config: DeployConfig, executable: str, tail: bool) -> None:
    ...
    with c.cd(config.remote_config.remote_root):
        ...
        with console.status("[yellow]Running executable on remote...", spinner="dots"):
            c.run(
                f"source ~/.bashrc && "
                f"export HF_HOME='/vol/bitbucket/$USER/.cache/huggingface' && "
...
def push_files(connection, config: RemoteConfig) -> None:
    ...
    with console.status("[yellow]Pushing files to remote...", spinner="dots"):
        connection.local(
            f"rsync -avz --exclude='__pycache__' --exclude='*.pyc' {' '.join(paths_to_transfer)} $USER@{config.hostname}:{config.remote_root}",
            hide=True,
        )
...
def submit_pbs_job(config: DeployConfig, tail_output=True):
    ...
    password = subprocess.check_output(["pass", "show", "cluster"], text=True).strip()

    with console.status("[yellow]Connecting to remote...", spinner="dots"):
        gateway = Connection("cluster", user="$USER")
        c = Connection(
